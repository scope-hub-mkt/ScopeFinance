...
-- ════════════════════════════════════════════════════════════════════
--  03/09/2026 — o deploy final: a origem do recebível e a senha
--  provisória (D-99, D-100, D-101)
--
--  ⛔ Coladas aqui DEPOIS de aplicadas no banco. O `schema.sql` continua
--  sendo o único original: as migrações datadas existem para o banco que
--  já roda, e o que vale para banco novo é isto.
-- ════════════════════════════════════════════════════════════════════

-- ── de 2026-09-03-origem-do-recebivel.sql ──
-- ════════════════════════════════════════════════════════════════════
...
-- ── de 2026-09-03-senha-provisoria.sql, com guarda ──
--
-- ⛔ **ACHADO ao colar esta migração aqui, em 03/09/2026 (`L-143`):** este
-- `schema.sql` **não cria `usuarios`**. A tabela nasce só em
-- `2026-08-31-usuarios.sql`, que nunca foi incorporada — de modo que o
-- arquivo que se apresenta como "o original" descreve um banco sem a tabela
-- de quem entra no sistema.
--
-- Medido: um `alter table usuarios` cru aqui derruba o arquivo inteiro em
-- banco vazio ("relation usuarios does not exist"), e o `schema.sql` passa a
-- ser inaplicável do zero — o mesmo defeito que a Dashboard já catalogou uma
-- vez, quando o bloco de `PBI-053` ficou no arquivo sem nunca chegar ao banco.
--
-- ⚖️ A guarda abaixo é **remendo declarado, não solução**: ela mantém o
-- arquivo aplicável enquanto a incorporação de `usuarios` não acontece. Sem
-- `usuarios`, a coluna simplesmente não nasce — e é isso que `L-143` registra,
-- para não ser redescoberto daqui a três meses.
do $$
begin
